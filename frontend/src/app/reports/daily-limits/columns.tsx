...
import { DailyLimitEntry } from "@/lib/data/dummy-daily-limits"
import { Badge } from "@/components/ui/badge"
import { cn } from "@/lib/utils"

export const columns: ColumnDef<DailyLimitEntry>[] = [
    {
    ...
    },
    {
        ...
        header: () => <div className="text-right font-bold text-black">Daily Limit</div>,
        cell: ({ row }) => <div className="font-medium text-gray-900">{row.original.dailyLimit} hrs</div>
    },
    {
    ...
    },
    {
        ...
        cell: ({ row }) => {
            const percentage = row.original.dailyLimit > 0
                ? (row.original.hoursTracked / row.original.dailyLimit) * 100
                : 0
    ...
    },
    {
        ...
        cell: ({ row }) => {
            const remaining = row.original.dailyLimit - row.original.hoursTracked
            return (
                <div className={cn(
                    "font-bold",
